...
export const ActivePlayers = ({ players, gameStarted }) => {
    ...
    return (
        <Flex direction='column' marginLeft='2' justifyContent='space-between' height='100%'>
            <Flex>
                <Flex

                    justifyContent='space-between' width='220px'>
                    {
                        gameStarted && players && players.map((player: any) => {
                            return (
                                <Flex
                                    key={player.id}
                                    p='4'
                                    ...
                                    <Heading as='h3' size='lg' color='#B7E778'>{player.name}</Heading>
                                </Flex>
                            )
                        })
                    }
            ...
            {
                gameStarted ? <Flex paddingTop='4'>
                    <CustomButton onClick={startGame}>Start Game</CustomButton>
                </Flex> : <CustomButton onClick={startGame}>Leave Game</CustomButton>
            }
        </Flex>
    )
}